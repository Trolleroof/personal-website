...
date: 2026-07-11
thumbnail: /blog/humanoid-penalty/thumbnail.png
excerpt: >-
  ...
  turned it into accurate shots to both corners.
clips:
...
I was recently watching the World Cup and naturally wondered if we could get humanoids to play soccer. Locomotion policies have been improving, and the Unitree G1 was the most capable humanoid form factor available to the public at the time. So why not try to get one to replicate the best players on the world's stage?
...
I only noticed the gap once I gave the virtual pitch real friction. Before that, weak kicks let the ball roll forever and hide the problem. With friction on, the ball stopped at 3.7 meters and I had to rethink the approach.
...
**Step 1:** I used a small PyTorch proxy so I could run gradients through it and search quickly. It guessed the rough shape of the kick — backswing timing, strike timing, small joint tweaks — in about 400 Adam steps, under a second on an MI300X. It only suggests kicks; it never decides whether they work.

**Step 2:** I ran Cross-Entropy Method (CEM) search in the full simulator. Each try adjusted a small control set:

- right hip, knee, and ankle at three moments around contact
- how hard the plant leg braces
- playback speed
- pelvis rotation

There were 48 candidates per round, for 12 rounds. I kept the best ones, sampled around them, and repeated.

I scored a kick as good if the ball crossed the goal line near the target with enough speed to matter, with a small penalty if the motion drifted too far from the human kick. It was faster than training a full policy from scratch — and MuJoCo physics always got the final vote.
...
The optimized left-corner kick reached 7.32 m/s and crossed 0.01 meters from its target. The center shot hit 9.94 m/s and crossed essentially dead on. The right-corner shot reached 5.92 m/s and also finished 0.01 meters from its target. All three came from MuJoCo rollouts in sim.
